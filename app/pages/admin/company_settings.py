...
def company_settings_page() -> None:
    ui.label('Firmenangaben & Vorlagen').classes('text-[24px] font-semibold text-[#1e3a5f] mb-4')

    state = {
        'profile_id': None,
        'name': '', 'street': '', 'zip_code': '', 'city': '',
        'phone': '', 'email': '', 'website': '',
        'iban': '', 'bank_name': '', 'logo_path': None
    }
    ...
    with get_session() as session:
        ...
        LOGO_DIR = (app_settings.upload_path_logos
                    if app_settings and app_settings.upload_path_logos
                    else './data/uploads/logos')

        TEMPLATE_DIR = (app_settings.upload_path_templates
        ...
        with get_session() as session:
            ...
            if profile:
                state.update({
                    ...
                    'name': profile.name or '', 'street': profile.street or '',
                    'zip_code': profile.zip_code or '', 'city': profile.city or '',
                    'phone': profile.phone or '', 'email': profile.email or '',
                    'website': profile.website or '', 'iban': profile.iban or '',
                    'bank_name': profile.bank_name or '', 'logo_path': profile.logo_path
                })
            else:
    ...
    def save_company_data():
        with get_session() as session:
            profile = session.query(CompanyProfile).filter_by(id=state['profile_id']).first()
            if profile:
    ...
    def get_upload_filename(e) -> str:
        ...
        if hasattr(e, 'name') and e.name: return e.name
        if hasattr(e, 'filename') and e.filename: return e.filename
    ...
    async def handle_logo_upload(e):
        ...
        if not file_bytes:
            return ui.notify('Fehler: Dateiinhalt konnte nicht gelesen werden.', type='negative')
    ...
    with ui.row().classes('w-full gap-8'):
        ...
        with ui.column().classes('flex-1 max-w-md gap-4'):
            with ui.card().classes('w-full p-6 shadow-sm border border-slate-200'):
                ...
                ui.input('Firmenname').bind_value(state, 'name').classes('w-full').props('outlined dense')
                ui.input('Strasse & Nr.').bind_value(state, 'street').classes('w-full').props('outlined dense')
                with ui.row().classes('w-full gap-2'):
                    ui.input('PLZ').bind_value(state, 'zip_code').classes('w-1/3').props('outlined dense')
                    ui.input('Ort').bind_value(state, 'city').classes('flex-1').props('outlined dense')

                ui.input('Telefon').bind_value(state, 'phone').classes('w-full').props('outlined dense')
                ui.input('E-Mail').bind_value(state, 'email').classes('w-full').props('outlined dense type="email"')
                ui.input('Webseite').bind_value(state, 'website').classes('w-full').props('outlined dense')

            with ui.card().classes('w-full p-6 shadow-sm border border-slate-200'):
                ui.label('Firmenlogo').classes('text-lg font-bold text-[#1e3a5f] border-b border-gray-100 pb-2 mb-2')
                ...
                def logo_preview():
                    if state['logo_path'] and os.path.exists(state['logo_path']):
                        ...
                        ui.label('Kein Logo hinterlegt').classes('text-gray-400 italic mb-2')

                logo_preview()
                ui.upload(on_upload=handle_logo_upload, auto_upload=True, max_files=1,
                          label='Neues Logo hochladen').props('accept="image/*" flat bordered')
        ...
        with ui.column().classes('flex-1 max-w-2xl gap-4'):
            with ui.card().classes('w-full p-6 shadow-sm border border-slate-200'):
                ui.label('Kontoinformationen').classes(
                    'text-lg font-bold text-[#1e3a5f] border-b border-gray-100 pb-2 mb-2')
                ui.input('Bankname').bind_value(state, 'bank_name').classes('w-full').props('outlined dense')
                ui.input('IBAN').bind_value(state, 'iban').classes('w-full').props('outlined dense')

                with ui.row().classes('w-full justify-end mt-4'):
                    ui.button('Stammdaten speichern', icon='save', on_click=save_company_data).props(
            ...
            with ui.card().classes('w-full p-6 shadow-sm border border-slate-200'):
                ...
                selected_type = ui.select(template_types, label='Zuweisungstyp für Upload').classes(
                    'w-full max-w-[300px] mb-2').props('outlined dense')

                @ui.refreshable
                def template_list():
                    with get_session() as session:
                        templates = session.query(DocumentTemplate).order_by(DocumentTemplate.doc_type).all()

                    if not templates:
                        ui.label('Noch keine Vorlagen hochgeladen.').classes('text-gray-400 italic')
                    else:
                            ...
                            for t in templates:
                                with ui.row().classes(
                                    ...
                                    with ui.row().classes('items-center gap-4'):
                                        ...
                                        ui.label(t.name).classes('font-medium text-sm text-slate-700')
                                    ui.button(icon='delete',
                                              on_click=lambda doc_id=t.id: delete_template(doc_id)).props(
                                        'flat round dense color="negative"')

                # WICHTIG: Die Funktion muss jetzt 'async def' sein!
                async def handle_template_upload(e):
                    if not selected_type.value:
                        return ui.notify('Bitte zuerst einen Vorlagentyp auswählen!', type='warning')
                    ...
                    with get_session() as session:
                        new_tpl = DocumentTemplate(doc_type=selected_type.value, name=filename, file_path=filepath,
                                                   is_active=True)
                        session.add(new_tpl)
                        session.commit()

                    ui.notify(f'Vorlage für {selected_type.value} hochgeladen!', type='positive')
                    template_list.refresh()

                def delete_template(doc_id):
                    with get_session() as session:
                        tpl = session.query(DocumentTemplate).filter_by(id=doc_id).first()
                        if tpl:
                ...
                ui.upload(on_upload=handle_template_upload, auto_upload=True, max_files=1,
                          label='Vorlagendatei hochladen').props('accept=".docx,.odt" flat bordered').classes(
                    'w-full mb-4')
                template_list()