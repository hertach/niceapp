...
def template_settings_page() -> None:
    ui.label('Vorlagenverwaltung').classes('text-[24px] font-semibold text-[#1e3a5f] mb-4')

    # Platzhalter-Definition
    PLACEHOLDERS = {
        'Allgemein (Firma)': ['{{firma_name}}', '{{firma_strasse}}', '{{firma_ort}}', '{{firma_iban}}',
                              '{{firma_bank}}'],
        'Patient': ['{{p_vorname}}', '{{p_nachname}}', '{{p_strasse}}', '{{p_plz}}', '{{p_ort}}', '{{p_geburtsdatum}}'],
        'Sitzung / Finanzen': ['{{s_datum}}', '{{s_betrag_netto}}', '{{s_mwst_satz}}', '{{s_betrag_brutto}}',
                               '{{s_anliegen}}']
    }

    with get_session() as session:
        app_settings = session.query(AppSetting).first()
        TEMPLATE_DIR = app_settings.upload_path_templates if app_settings and app_settings.upload_path_templates else './data/uploads/templates'
    ...
    with edit_dialog, ui.card().classes('min-w-[400px] p-6'):
        ...
        ui.input('Anzeigename / Dateiname').bind_value(edit_state, 'name').classes('w-full mb-2').props(
            'outlined dense')
        ui.select(['Rechnung', 'Quittung', 'Begleitbrief', 'Mahnung'], label='Zuweisungstyp').bind_value(edit_state,
                                                                                                         'doc_type').classes(
            'w-full mb-2').props('outlined dense')
        ui.checkbox('Als Standard für diesen Typ festlegen').bind_value(edit_state, 'is_default').classes('mb-4')

        def save_edit():
            with get_session() as session:
                t = session.query(DocumentTemplate).filter_by(id=edit_state['id']).first()
                if t:
        ...
        with ui.row().classes('w-full justify-end gap-2'):
            ui.button('Abbrechen', on_click=edit_dialog.close).props('flat text-color="grey"')
            ui.button('Speichern', on_click=save_edit).props('unelevated color="primary"')
    ...
    def template_table_refresh():
        ...
        columns = [
            {'name': 'doc_type', 'label': 'Typ', 'field': 'doc_type', 'align': 'left', 'sortable': True},
            {'name': 'name', 'label': 'Dateiname / Bezeichnung', 'field': 'name', 'align': 'left', 'sortable': True},
            {'name': 'is_default', 'label': 'Standard', 'field': 'is_default', 'align': 'center', 'sortable': True},
        ...
        table.add_slot('body-cell-is_default', r'''
            <q-td :props="props">
        ...
        table.on('set_default', lambda msg: set_default(msg.args['id'], msg.args['doc_type']))
    ...
    async def handle_upload(e):
        ...
        safe_filename = f"{selected_type.value}_{timestamp}_{original_name.replace(' ', '_')}"
        ...
        with get_session() as session:
            existing_default = session.query(DocumentTemplate).filter_by(doc_type=selected_type.value,
    ...
    def set_default(tpl_id, t_type):
        with get_session() as session:
            session.query(DocumentTemplate).filter_by(doc_type=t_type).update({DocumentTemplate.is_default: False})
            session.query(DocumentTemplate).filter_by(id=tpl_id).update({DocumentTemplate.is_default: True})
            session.commit()
        template_table_refresh.refresh()
    ...
    def delete_tpl(tpl_id):
        with get_session() as session:
            ...
            if t:
                if os.path.exists(t.file_path): os.remove(t.file_path)
                session.delete(t)
                session.commit()
        template_table_refresh.refresh()
    ...
    with ui.row().classes('w-full gap-8 items-start'):
        with ui.column().classes('flex-1 min-w-0'):
            # Upload Card
            with ui.card().classes('w-full p-6 mb-6 shadow-sm border border-slate-200 bg-blue-50/20'):
                ui.label('Vorlage hochladen').classes('font-bold text-[#1e3a5f] mb-2')
                with ui.row().classes('items-center w-full gap-4'):
                    selected_type = ui.select(['Rechnung', 'Quittung', 'Begleitbrief', 'Mahnung'],
                                              label='Zuweisung').classes('w-48').props('outlined dense bg-white')
                    ui.upload(on_upload=handle_upload, auto_upload=True, label='Datei wählen').props(
                        'flat bordered bg-white accept=".docx,.odt"').classes('flex-1')

            template_table_refresh()

        # Platzhalter-Liste Rechts
        with ui.column().classes('w-80 shrink-0'):
            with ui.card().classes('w-full p-6 shadow-sm border border-slate-200 sticky top-4'):
                ui.label('Mögliche Platzhalter').classes('font-bold text-lg text-[#1e3a5f] mb-1')
                ui.label('Klicke auf ein Icon, um den Tag zu kopieren.').classes('text-xs text-slate-500 mb-6')
                ...
                for cat, tags in PLACEHOLDERS.items():
                    ui.label(cat).classes('text-sm font-bold mt-2 border-b w-full pb-1 text-slate-700')
                    for tag in tags:
                        with ui.row().classes('w-full justify-between items-center py-1 group'):
                            ui.label(tag).classes('text-xs font-mono text-[#0078d4]')
                            ui.button(icon='content_copy', on_click=lambda t=tag: copy_tag(t)).props(
                                'flat round dense size=xs color="grey"').classes(
