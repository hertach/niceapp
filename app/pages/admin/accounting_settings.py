...
def accounting_settings_page() -> None:
    ui.label("Buchhaltung: Kontenplan & Perioden").classes(
    ...
    def confirm_delete(row_data: dict):
        with ui.dialog() as dialog, ui.card().classes("p-6 w-[400px]"):
            ...
            ui.label(
                f"Möchtest du das Konto '{row_data['number']} - {row_data['name']}' wirklich löschen?"
            ...
            with ui.row().classes("w-full justify-end gap-2"):
                ui.button("Abbrechen", on_click=dialog.close).props("flat")
                ui.button("Löschen", on_click=do_delete).props(
    ...
    def account_table():
        with get_session() as session:
            accounts = (
                session.query(Account).order_by(asc(Account.account_number)).all()
            )
    ...
    with ui.row().classes("w-full gap-4 mb-6"):
        with ui.card().classes(
            "flex-1 p-6 shadow-sm border border-slate-200 bg-blue-50/20"
        ):
            ui.label("Neues Konto hinzufügen").classes("font-bold mb-2 text-[#1e3a5f]")
            with ui.row().classes("w-full gap-4 items-end"):
                acc_num = (
                    ui.number("Konto-Nummer", format="%.0f")
                    .classes("w-32")
                    .props("outlined dense bg-white")
                )
                acc_name = (
                    ui.input("Bezeichnung")
                    .classes("flex-1")
                    .props("outlined dense bg-white")
                )
                ...
                def add_account():
                    ...
                    with get_session() as session:
                        ...
                        existing = (
                            ...
                            .filter_by(account_number=int(acc_num.value))
                            .first()
                        )
                        if existing:
                            return ui.notify(
                                f"Konto {int(acc_num.value)} existiert bereits!",
                                type="negative",
                            )
                        ...
                        a_group = (
                            int(num_str[:2]) if len(num_str) >= 2 else a_class * 10
                        )
                ...
                ui.button("Hinzufügen", icon="add", on_click=add_account).props(
                    "unelevated"
