...
def patients_page(navigate) -> None:
    ui.label('Patientenverwaltung').classes('text-[24px] font-semibold text-[#1e3a5f] mb-4')
    ...
    with ui.row().classes('w-full justify-between items-center mb-6'):
        # Suchfeld
        search_input = ui.input('Patient suchen...').classes('w-[300px]').props('outlined dense clearable')
        with search_input.add_slot('append'):
            ui.icon('search')

        # Button: Neuer Patient
        ui.button('Neuer Patient', icon='add', on_click=lambda: open_patient_detail(None)).props('unelevated').classes(
            'bg-[#0078d4] text-white px-4')
    ...
    def fetch_patients():
        with get_session() as session:
            ...
            if search_input.value:
                ...
                query = query.filter(Patient.first_name.ilike(search_term) | Patient.last_name.ilike(search_term))
            ...
            return [
                {
                    ...
                    'birthdate': p.birthdate.strftime('%d.%m.%Y') if p.birthdate else '-',
                    'gender': p.gender or '-'
                } for p in patients
            ]
    ...
    columns = [
        ...
        {'name': 'birthdate', 'label': 'Geburtsdatum', 'field': 'birthdate', 'align': 'left'},
    ...
    table = ui.table(
        ...
        rows=fetch_patients(),
        row_key='id'
    ).classes('w-full shadow-sm border border-slate-200')
