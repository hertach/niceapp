...
class DirectoryPicker(ui.dialog):
    def __init__(self, initial_path: str = "."):
        ...
        with self, ui.card().classes("w-[500px] h-[600px] flex flex-col p-6"):
            ui.label("Verzeichnis auswählen").classes(
                "text-[18px] font-semibold text-[#1e3a5f] mb-2"
            )

            # Zeigt den aktuell ausgewählten Pfad an
            self.path_label = ui.label(str(self.path)).classes(
                "text-[13px] text-gray-500 mb-4 truncate w-full"
            )
            ...
            with ui.row().classes("w-full justify-end mt-4 gap-2"):
                ui.button("Abbrechen", on_click=self.close).props(
                    'flat text-color="grey"'
                )
                ui.button(
                    "Auswählen", on_click=lambda: self.submit(str(self.path))
        ...
        with self.container:
            ...
            if self.path.parent != self.path:
                with (
                    ui.row()
                    .classes(
                        "items-center gap-2 cursor-pointer hover:bg-blue-50 w-full p-2 rounded transition-colors"
                    )
                    .on("click", lambda: self.navigate(self.path.parent))
                ):
            ...
            try:
                ...
                for d in sorted(dirs, key=lambda x: x.name.lower()):
                    with (
                        ...
                        .on("click", lambda p=d: self.navigate(p))
                    ):
