...
class FiscalYear(Base):
    ...
    )  # True = Jahr ist abgeschlossen, keine Buchungen mehr möglich
...
class Account(Base):
    ...
    )  # 1=Aktiven, 2=Passiven, 3=Betriebsertrag, etc.
...
class JournalEntry(Base):
    ...
    fiscal_year_id = Column(
        Integer, ForeignKey("accounting_fiscal_years.id"), nullable=False
    )

    patient_session_id = Column(
        Integer, ForeignKey("patient_sessions.id"), nullable=True
    )
    date = Column(Date, nullable=False, default=date.today)
    description = Column(
        String(255), nullable=False
    )  # Buchungstext, z.B. "Rechnung Meier"
...
class JournalEntryLine(Base):
    ...
    journal_entry_id = Column(
        Integer, ForeignKey("accounting_journal_entries.id"), nullable=False
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<JournalEntryLine Acc:{self.account_id} S:{self.debit} H:{self.credit}>"
        )
